...
var fields = [
    new PiFieldDescriptor_1.PiFieldDescriptor('name', 'string', true),
    new PiFieldDescriptor_1.PiFieldDescriptor('when', 'date', false),
    new PiFieldDescriptor_1.PiFieldDescriptor('age', 'integer', true),
    new PiFieldDescriptor_1.PiFieldDescriptor('isMale', 'boolean', true),
];
var fd;
describe('PiFieldDescriptor', function () {
    describe('constructors', function () {
        it('Created with name and numeric data (string, required)', function () {
            ...
            expect(fd.type).toEqual('string');
        });
        it('Created with name and numeric data overriding "required"', function () {
            fd = new PiFieldDescriptor_1.PiFieldDescriptor('name', lib_1.typeBitValue.string | lib_1.FieldData.required, false);
            expect(fd).toBeTruthy();
            expect(fd.name).toEqual('name');
        ...
        it('Created with name and numeric data (integer, not required)', function () {
            ...
            expect(fd.name).toEqual('name');
            expect(fd.required).toEqual(false);
            expect(fd.type).toEqual('integer');
        });
        it('Created with name, type=integer, required=false', function () {
            fd = new PiFieldDescriptor_1.PiFieldDescriptor('name', 'integer', false);
            expect(fd).toBeTruthy();
            expect(fd.name).toEqual('name');
            expect(fd.required).toEqual(false);
            expect(fd.type).toEqual('integer');
        ...
        it('Created with field:PiField', function () {
            var field = { name: 'name', required: true, isArray: true, comment: 'xpto', type: 'date' };
        ...
        it('Created enum without values', function () {
            var field = { name: 'xpto', required: true, isArray: true, type: 'enum' };
        ...
        it('Invalid value for type parameter', function () {
            ...
            expect(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('field12', d); })
                .toThrowError(/PiFieldDescriptor.constructor: Invalid type '.*'/);
        });
    });
    describe('Miscelaneus methods', function () {
        var init = function (req) { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('name', 'integer', req); };
    ...
    describe('Required String Validation', function () {
        var fd;
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('name', 'string', true); });
    ...
    describe('Required Date Validation', function () {
        var fd;
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('birthdate', 'date', true); });
        it('with number: no error', function () {
            expect(function () { return fd.validate(1); }).not.toThrow();
        });
    ...
    describe('Cast To String', function () {
        var fd;
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('address', 'string', true); });
    ...
    describe('Cast To Enum', function () {
        var test = function (caption, val, result, error) {
            it(caption, function () {
                var data = { sex: val };
                var fd = new PiFieldDescriptor_1.PiFieldDescriptor('sex', 'enum', true, ['male', 'female']);
                if (error)
    ...
    describe('Cast To Number', function () {
        ...
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('weight', 'number', true); });
        var test = function (val, res) {
            var d = { weight: val };
    ...
    describe('Cast To Integer', function () {
        ...
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('age', 'integer', true); });
        var test = function (val, res) {
            var d = { age: val };
            if (typeof res == 'number') {
                fd.cast(d);
    ...
    describe('Cast To Date', function () {
        ...
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('birthdate', 'date', false); });
        var test = function (val, res) {
            var d = { birthdate: val };
    ...
    describe('Cast To Boolean', function () {
        var fd;
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('isStudent', 'boolean', true); });
        var test = function (val, res) {
            var d = { isStudent: val };
            fd.cast(d);
    ...
    describe('Cast To Any', function () {
        var fd;
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('tag', 'any', true); });
    ...
    describe('Cast To Array<Date>', function () {
        var fd;
        var err = /cannot be cast to a date/;
        beforeEach(function () { return fd = new PiFieldDescriptor_1.PiFieldDescriptor('datearray', 'date[]', true); });
        var test = function (val, res) {
            var d = { datearray: val };
